...
namespace grad {
    namespace replay {
        ...
        public:
            ...
            Record(std::string icon, int color, Position position, float direction, std::string name, std::string group, std::optional<Position> target, const std::map<int, std::string>& colorMap);
            Record(types::auto_array<types::game_value> record, std::shared_ptr<Record> prevPtr, const std::map<int, std::string>& colorMap);
        };
